...
total_replicates = 3*global_replicates + \
    3*global_replicates_short_internalbranch
...
print("global_min_short_internal_branchlength: ",
      global_min_short_internal_branchlength)
print("global_max_short_internal_branchlength: ",
    ...
    with open(file_name, "w") as tree_file:
        if topology == 0:
            string = "1.0  {0} {1} ((A:{2},B:{3}):{4},(C:{5},D:{6}):{7})\n".format(
                seqlen, modelname, B1, B2, B3, B4, B5, B6)
        if topology == 1:
    ...
    with open(file_name, "w") as model_file:
        if model == 'JC':
            ...
            string = 'basefreq: {0}  {1}  {2}  {3}\n'.format(
                basefreq_1, basefreq_2, basefreq_1, basefreq_2)
            model_file.write(string)
            model_file.write("end model\n")
        elif model == 'HKY':
            model_file.write("begin model\n")
            ...
            model_file.write(string)
            string = 'rrates: {} {} {} {} {} {}\n'.format(
                GTR_A_C, GTR_A_G, GTR_A_T, GTR_C_G, GTR_C_T, GTR_G_T)
            model_file.write(string)
            model_file.write("end model\n")
...
def simulate_random_lengths_random_free_model(topology, seed, base_model_list, min_brlen, max_brlen, min_internal_brlen,
    ...
    model_filename = tmpdir + "Modelfile_free" + \
        runID + "_" + str(seed) + ".txt"
    ...
    print("Calling make_model_file: ", model_filename, " ",
          model, " ", shape, " ", pinv, " ", modelname)
    make_model_file(model_filename, model, shape, pinv, tstv, basefreq_1, basefreq_2,
                    GTR_A_C, GTR_A_G, GTR_A_T, GTR_C_G, GTR_C_T, GTR_G_T, modelname)
    make_tree_file_model(sim_filename, topology, r1, r2,
                         r3, r4, r5, r6,  seqlen, modelname)

    all_sim_filename = "all_sim_tree" + runID + ".txt"
    os.system("echo " + str(seed) + " >> " + all_sim_filename)
    os.system("type " + sim_filename + " >> " + all_sim_filename)
...
def branched_model_1(hparam, normlayer):
    ...
    x = tf.keras.layers.Dense(
        units=256, activation=hparam[HP_ACTIVATION])(inputn)
...
def branched_model_2(hparam, normlayer):
    ...
    z = tf.keras.layers.Dense(
        units=128, activation=hparam[HP_ACTIVATION])(combined)
    z = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(z)
    z = tf.keras.layers.Dense(
        units=64, activation=hparam[HP_ACTIVATION])(combined)
    z = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(z)
...
def branched_model_3(hparam, normlayer):
    ...
    combined = tf.keras.layers.concatenate(
        [x_ann.output, y_ann.output, w_ann.output])

    z = tf.keras.layers.Dense(
        units=256, activation=hparam[HP_ACTIVATION])(combined)
    z = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(z)
...
def branched_model_10(hparam, normlayer):
    ...
    x5 = tf.keras.layers.Dense(
        units=32, activation=hparam[HP_ACTIVATION])(inputn)
    ...
    x6 = tf.keras.layers.Dense(
        units=64, activation=hparam[HP_ACTIVATION])(inputn)
    ...
    x7 = tf.keras.layers.Dense(
        units=128, activation=hparam[HP_ACTIVATION])(inputn)
    ...
    x8 = tf.keras.layers.Dense(
        units=111, activation=hparam[HP_ACTIVATION])(inputn)
...
def branched_model_1u(hparam, normlayer):
    ...
    x10 = tf.keras.layers.Dense(
        units=256, activation=hparam[HP_ACTIVATION])(x9)
    x11 = tf.keras.layers.Dense(
        units=256, activation=hparam[HP_ACTIVATION])(x10)
    x14 = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(x11)
    x15 = tf.keras.layers.Dense(
        units=128, activation=hparam[HP_ACTIVATION])(x14)
    x16 = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(x15)
    x17 = tf.keras.layers.Dense(
        units=64, activation=hparam[HP_ACTIVATION])(x16)
    x18 = tf.keras.layers.Dense(
        units=32, activation=hparam[HP_ACTIVATION])(x17)
    x19 = tf.keras.layers.Dense(
        units=16, activation=hparam[HP_ACTIVATION])(x18)
    if args.sequence_type == 'DNA':
...
def branched_model_cu(hparam, normlayer):
    ...
    x10 = tf.keras.layers.Dense(
        units=256, activation=hparam[HP_ACTIVATION])(y1)

    y2 = tf.keras.layers.concatenate([x10, x3])

    x11 = tf.keras.layers.Dense(
        units=256, activation=hparam[HP_ACTIVATION])(y2)
    x14 = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(x11)
    x15 = tf.keras.layers.Dense(
        units=128, activation=hparam[HP_ACTIVATION])(x14)
    x16 = tf.keras.layers.Dropout(hparam[HP_DROPOUT])(x15)
    x17 = tf.keras.layers.Dense(
        units=64, activation=hparam[HP_ACTIVATION])(x16)
    x19 = tf.keras.layers.Dense(
        units=16, activation=hparam[HP_ACTIVATION])(x17)
    if args.sequence_type == 'DNA':
        x20 = tf.keras.layers.Dense(units=5, activation='softmax')(x19)
    elif args.sequence_type == 'AA':
        x20 = tf.keras.layers.Dense(units=4, activation='softmax')(x19)
...
def run_hparam_one_model_one_hset(model_func, X_train, Y_train, X_test, Y_test, hparams, normlayer, do_probabilities=False):
    ...
    if args.sequence_type == 'DNA':
        logdir = 'DNAmodelpred' + '_' + \
            str(args.epochs) + '_' + args.neural_network + '.log'

    elif args.sequence_type == 'AA':
        logdir = 'AAmodelpred' + '_' + \
            str(args.epochs) + '_' + args.neural_network + '.log'
    ...
    for activation in HP_ACTIVATION.domain.values:
        for dropout in HP_DROPOUT.domain.values:
            for optimizer in HP_OPTIMIZER.domain.values:
                ...
                fitted_model = run_hparam_one_model_one_hset(
                    model_func, X_train, Y_train, X_test, Y_test, hparams, normlayer)
                ...
                if args.sequence_type == 'DNA':
                    ...
                    fitted_model.save(
                        'DNAmodelpred' + '_' + str(args.epochs) + '.' + args.neural_network)
                    print('The name of the saved NN: DNAmodelpred' +
                          '_' + str(args.epochs) + '.' + args.neural_network)
                elif args.sequence_type == 'AA':
                    print_highest_likelihood_evaluation(fitted_model, temporary_JTT,
                                                        temporary_LG,
                                                        temporary_WAG, temporary_DAY)
                    fitted_model.save(
                        'AAmodelpred' + '_' + str(args.epochs) + '.' + args.neural_network)
                    print('The name of the saved NN: AAmodelpred' +
                          '_' + str(args.epochs) + '.' + args.neural_network)
                session_num += 1
