...
parser.add_argument('-NN_name', type=str,
                    help="Compulsory argument. Enter the name of the NN folder.", required=True)
parser.add_argument('-alignment_file', type=str,
                    help="Compulsory argument. Enter the name of the multiplealignment file.", required=True)
...
def is_quartet_counter_available():
    try:
        subprocess.check_output(['where', 'quartet-pattern-counter-v1.1'])
        return True
...
def tree_topology(topology, m):
    if topology == 0:
        ...
        string = "((" + str(header_list[0]) + "," + str(header_list[1]) + \
            "),(" + str(header_list[2]) + "," + str(header_list[3]) + "))"
        print("Model:", m, " Tree topology: ", string)

    elif topology == 1:
        header_list = read_alignment_file(args.alignment_file)
        string = "((" + str(header_list[0]) + "," + str(header_list[2]) + \
            "),(" + str(header_list[1]) + "," + str(header_list[3]) + "))"
        print("Model:", m, " Tree topology: ", string)

    elif topology == 2:
        header_list = read_alignment_file(args.alignment_file)
        string = "((" + str(header_list[0]) + "," + str(header_list[3]) + \
            "),(" + str(header_list[2]) + "," + str(header_list[1]) + "))"
        print("Model:", m, " Tree topology: ", string)

    elif topology == 3:
        header_list = read_alignment_file(args.alignment_file)
        string = "((" + str(header_list[0]) + "," + str(header_list[1]) + \
            "),(" + str(header_list[3]) + "," + str(header_list[2]) + "))"
        print("Model:", m, " Tree topology: ", string)

    elif topology == 4:
        header_list = read_alignment_file(args.alignment_file)
        string = "((" + str(header_list[0]) + "," + str(header_list[3]) + \
            "),(" + str(header_list[1]) + "," + str(header_list[2]) + "))"
        print("Model:", m, " Tree topology: ", string)
...
if args.sequence_type == 'DNA':
    # command = 'quartet-pattern-counter-v1.1.exe ' + \
    #     alignment_file + " " + os.getcwd() + "/out.npy"
    executable_path = 'quartet-pattern-counter-v1.1.exe'
    # Build the command as a string
    command = f'"{executable_path} " "{alignment_file} " "out.npy"'
elif args.sequence_type == 'AA':
    command = 'quartet-pattern-counter-v1.1 -p ' + \
        alignment_file + " " + os.getcwd() + "/out.npy"
...
y = x.item()
print("topology number is " + str(y))
